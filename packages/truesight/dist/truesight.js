...
(function(global, factory) {
  ...
  function findLongestAxis(vbox) {
    ...
    const longestAxis = rgbColorAxes.reduce(
      (previous, current) => (current.width > previous.width ? current : previous)
    );
  ...
  var validatePopularityParameters = async function validatePopularityParameters(parameters) {
    ...
    if (!VALID_SATURATION_REGION_SIZES.liesIn(saturation)) {
      throw new RangeError(
        `saturation in regionSize property should lie in ${VALID_SATURATION_REGION_SIZES.toString()}`
      );
    }
