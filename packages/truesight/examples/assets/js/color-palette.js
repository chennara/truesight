window.onload = () => {
  ...
  imageElements.forEach((imageElement) => {
    ...
    const createSectionNode = (fn) => {
      const t0 = performance.now();
      const colorPalette = truesight[fn](parameters); // eslint-disable-line no-undef
    ...
    ['reduce', 'popularize'].forEach((fn) => {
      const sectionNode = createSectionNode(fn);
